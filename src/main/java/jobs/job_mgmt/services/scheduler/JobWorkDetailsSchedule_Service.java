...
public class JobWorkDetailsSchedule_Service implements I_JobWorkDetailsSchedule_Service {
	...
	public synchronized void scheduleJobs() {
		...
		CopyOnWriteArrayList<ServiceWorkMaster> servWorkMasters = serviceWorkReadRepo
		...
		if (servWorkMasters != null) {
			for (int i2 = 0; i2 < servWorkMasters.size(); i2++) {
				servWorkSeqNo = servWorkMasters.get(i2).getServiceWorkSeqNo();
				jobTemplateSeqNo = servWorkMasters.get(i2).getJobTemplateSeqNo();
				mgr_seq_no = servWorkMasters.get(i2).getCreatedBy();
				startDateTime = formatter.format(servWorkMasters.get(i2).getOnDate().toLocalDateTime());
				opFlag = servWorkMasters.get(i2).getOpFlag();
				this.doJobsForTemplate(servWorkSeqNo, jobTemplateSeqNo, mgr_seq_no, startDateTime, opFlag);
				logger.info("service work  :" + servWorkSeqNo);
				serviceWorkCUDRepo.updJobAllocStatus(servWorkSeqNo, 'Y');
	...
	public synchronized void doJobsForTemplate(Long servWorkSeqNo, Long jobTemplateSeqNo, Long mgr_seq_no,
		...
		for (int l = 0; l < jobMasters2.size(); l++) {
			...
			jobTripDetails = jobTripRepo.checkJobDetails(jobWorkSeqNo);

			if (jobTripDetails != null) {
				jobTripRepo.delJobDetails(jobWorkSeqNo);
			}
		}
	...
	private CopyOnWriteArrayList<JobWorkDetail_DTO> processJobWorkDetailsForMaster(Long servWorkSeqNo,
			Long jobTemplateSeqNo, String startDateTime, Integer opFlag, Long mgrSeqNo) {
		...
		if (jobsTemplateDetails != null) {
			...
			for (int i = 0; i < jobsTemplateDetails.size(); i++) {
				...
				jobTypeStructureDetails = jobStructureTargetDetailsReadRepo.getSelectJobStructureTargetDetailsForParent(
						jobsTemplateDetails.get(i).getId().getJOB_SEQ_NO(),
			...
			for (int i = 0; i < jobsTemplateDetails.size(); i++) {
				...
				jobDetails4 = jobWorkDetailsCUDRepo.save(jobDetails5);

				if (jobsTemplateDetails.get(i).getJobType() != null
						&& jobsTemplateDetails.get(i).getJobType().toUpperCase().equals("MOVE")) {
					JobTripTemplateDetail jobTripTemplateDetail = jobTripsTemplateRepo.getJobTripsStatusTemplate(
							jobsTemplateDetails.get(i).getId().getJobTemplateSeqNo(),
							jobsTemplateDetails.get(i).getId().getJOB_SEQ_NO());

					if (jobTripTemplateDetail != null) {
						jobTripReturn_DTO = setTripDetailsFromMaster(jobDetails4.getJobWorkSeqNo(),
								jobDetails4.getJobSeqNo(), jobsTemplateDetails.get(i).getId().getJobTemplateSeqNo(),
								endDate, opFlag);
						...
						if (opFlag.compareTo(Integer.valueOf(1)) == 0) {
							...
							jobWorkDetailsCUDRepo.updPlanStartAndEnd(currDate, endDate, jobDetails4.getJobWorkSeqNo());
							logger.info("plan start :" + currDate);
							logger.info("plan end :" + endDate);
						} else {
							jobWorkDetailsCUDRepo.updActualStartAndEnd(currDate, endDate,
									jobDetails4.getJobWorkSeqNo());
						}
	...
	private JobTripReturn_DTO setTripDetailsFromMaster(Long jobSeqNo, Long job_seq_no, Long template_seq_no,
